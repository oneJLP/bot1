                ...
                try:
                    ...
                    if rks < 13.5:
                        ...
                        await message.channel.send('欢迎>' + uname + '<前来审核！\n' + testify(rks) + (
                            '\n你的一次获取考题机会已耗尽，无法再次获取\n\n打完歌之后请使用/upload命令上传你的消息记录截图和结算截图！' if not isadmin else ''),mention_author=member)
...
大群群号730173176''')
                    ...
                    print(e)
                    await message.channel.send('输入格式出现错误或该序号不存在！',mention_author=member)
            else:
                await message.channel.send('你的权限不足！',mention_author=member)
            ...
            if isadmin:
                ...
                try:
                    ...
                    await message.channel.send('你的审核内容不合格，没有通过审核',mention_author=message.guild.get_member(disallow1_uid))
                except Exception as e:
                    print(e)
                    await message.channel.send('输入格式出现错误或该序号不存在！',mention_author=member)
            else:
